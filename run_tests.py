...
def bless_rcc_output(cfg: RunConfig) -> int:
    """Rewrite compiler/tests/*.output.expect using rcc + Python asm + Python sim (canonical)."""
    rcc = cfg.rcc_path
    if not rcc:
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="comma-separated suites: rcc, asm, examples (default: all three)",
    )
    args = ap.parse_args()

    only_parts = {x.strip() for x in args.only.split(",") if x.strip()}
    for p in only_parts:
        if p not in ("rcc", "asm", "examples"):
